...
export function Menu() {
  ...
  const { setAlunoSelecionado } = useAlunoContext();
            ...
            {alunos.map((aluno) => (
              ...
              <div key={aluno.id} className="bg-gray-100 border border-gray-300 rounded-lg shadow p-6 mb-2 flex items-center">
                ...
                <div className="flex gap-4 items-center">
                  <button onClick={() => handleEditar(aluno.id)} className="text-xs p-1 h-9 gap-2 mr-1">
                    <Link to={`/edicao/${aluno.id}`} >
                      <Pencil size={20} color="purple" /> 
                    </Link>
                  </button>
                  <button onClick={() => handleDeletar(aluno.id)} className="text-xs p-1 h-9 gap-2 mr-4">
